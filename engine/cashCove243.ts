export type SymbolCode = "A" | "K" | "Q" | "J" | "10" | "9" | "F1" | "F2" | "F3" | "F4" | "W" | "S";

export interface WeightedChoice<T extends number | string> {
  value: T;
  weight: number;
}
...
export interface EngineConfig {
  ...
  scatterSymbol: SymbolCode;
  wildSymbol: SymbolCode;
  triggerScatterCount: number;
  freeSpinsAwarded: number;
  allowRetriggerInFreeSpins: boolean;
  multiplierWeights: WeightedChoice<number>[];
}
...
export interface RoundOutcome {
  ...
  totalSpinsResolved: number;
  firstSpin: SpinOutcome;
  freeSpinOutcomes: SpinOutcome[];
}
...
export function playPaidSpinRound(
  rng: () => number,
  state: SessionState,
  betPerSpin = 1,
...
export function spinCashCove(
  ...
  const scatterPayMultiplier = config.scatterPaytable?.[scatterCount as 3 | 4 | 5] ?? 0;
...
export function buildWindow(rng: () => number, config: EngineConfig): SymbolCode[][] {
...
export function evaluate243Ways(
  window: SymbolCode[][],
  paytable: EngineConfig["paytable"],
  betPerSpin: number,
  wildSymbol: SymbolCode,
  scatterSymbol: SymbolCode,
): { totalPayout: number; winningWays: WinningWay[] } {
  ...
  for (const symbol of Object.keys(paytable)) {
    if (symbol === scatterSymbol || symbol === wildSymbol) continue;
    ...
    for (let reelIdx = 0; reelIdx < window.length; reelIdx += 1) {
      ...
      if (matchesOnReel === 0) break;
      count += 1;
      ways *= matchesOnReel;
    }

    if (count < 3) continue;
    const payMultiplier = paytable[symbol][count as 3 | 4 | 5] ?? 0;
...
export function countSymbols(window: SymbolCode[][], symbol: SymbolCode): number {
  let count = 0;
  for (const reel of window) {
    for (const reelSymbol of reel) {
      if (reelSymbol === symbol) count += 1;
    }
  }
  return count;
}

export function drawWeightedValue<T extends number | string>(rng: () => number, table: WeightedChoice<T>[]): T {
