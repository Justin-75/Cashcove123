...
import { createDefaultSessionState, spinCashCove, type SessionState } from "../engine/cashCove243.js";
...
const server = createServer(async (req, res) => {
  ...
  if (req.method === "POST" && url.pathname === "/api/deposit") {
    ...
    const payload = JSON.parse(body) as { sessionId?: string; amountCents?: number };
  ...
  if (req.method === "POST" && url.pathname === "/api/spin") {
    const body = await readBody(req);
    const payload = JSON.parse(body) as { sessionId?: string; betCredits?: number; denominationDollars?: 1 | 2 };
    const sessionId = payload.sessionId ?? "default";
...
async function readBody(req: { on: (event: string, handler: (chunk?: unknown) => void) => void }): Promise<string> {
