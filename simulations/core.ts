...
export type EngineSpin = (rng: () => number, spinIndex: number) => EngineSpinResult | Promise<EngineSpinResult>;

export interface SimulationConfig {
  spins: number;
  betPerSpin: number;
  topWinThresholdMultiplier: number;
  seed: number;
}
...
export interface SimulationMetrics {
  ...
  bonusRoundRate: number;
  freeSpinRate: number;
  freeSpinsPerTrigger: number;
  averageBonusWin: number;
  ...
  topWinThreshold: number;
  topWinFrequency: number;
  averageFreeSpinMultiplier: number;
  highMultiplierSpinRate: number;
  multiplierHistogram: Record<string, number>;
  p50Win: number;
  p90Win: number;
  p99Win: number;
}
...
export async function runSimulationBatch(
  ...
  if (config.spins <= 0) throw new Error("spins must be > 0");
  if (config.betPerSpin <= 0) throw new Error("betPerSpin must be > 0");

  const rng = createSeededRng(config.seed);
  const wins: number[] = [];
  const multiplierHistogram: Record<string, number> = {};
  ...
  for (let i = 0; i < config.spins; i += 1) {
    ...
    if (featureWin > 0) featureHits += 1;

    if (result.isBonusTrigger) bonusTriggers += 1;
    ...
    if (explicitMultipliers.length > 0) {
      ...
      for (const multiplier of explicitMultipliers) {
        ...
        if (multiplier >= 5) highMultiplierSpins += 1;
...
export function metricsToCsvRow(metrics: SimulationMetrics): string {
  return [
    metrics.spins,
    metrics.seed,
    metrics.betPerSpin,
...
export const CSV_HEADER =
  "spins,seed,betPerSpin,wagered,totalReturned,rtp,baseRtp,featureRtp,hitRate,featureHitRate,bonusTriggerRate,bonusRoundRate,freeSpinRate,freeSpinsPerTrigger,averageBonusWin,averageWinPerSpin,averageWinOnHit,stdDevWin,coefficientOfVariation,maxWin,maxWinMultiplier,topWinThreshold,topWinFrequency,averageFreeSpinMultiplier,highMultiplierSpinRate,multiplierHistogram,p50Win,p90Win,p99Win";
