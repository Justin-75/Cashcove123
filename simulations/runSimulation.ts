...
import { CSV_HEADER, metricsToCsvRow, runSimulationBatch, type EngineSpin } from "./core.js";
...
async function resolveEngine(engineModule: string, bet: number): Promise<EngineSpin> {
  ...
  if (typeof candidate !== "function") {
    throw new Error(
      "Engine module must export a named `spin(rng, spinIndex)` function that returns { totalWin, hit? }",
    );
  }
  return candidate as EngineSpin;
}
...
function numArg(value: string | undefined, fallback: number): number {
  if (!value) return fallback;
